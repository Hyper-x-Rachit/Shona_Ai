...
async def restart(client, m: Message):
    if m.chat.type == "private":
        ...
        await m.reply_photo(
            ...
            reply_markup=InlineKeyboardMarkup(Rachit_op),
        )
...
async def cb_handler(Client, query: CallbackQuery):
    Rachitdb = MongoClient(MONGO_URL)
    Rachit = Rachitdb["RachitDb"]["Rachit"]
    if query.data == "HELP":
        ...
        if query.from_user.id not in (await is_admins(query.message.chat.id)):
            ...
            is_Rachit = Rachit.find_one({"chat_id": query.message.chat.id})
            if not is_Rachit:           
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_Rachit:
                Rachit.delete_one({"chat_id": query.message.chat.id})
        ...
        if query.from_user.id not in (await is_admins(query.message.chat.id)):
            ...
            is_Rachit = Rachit.find_one({"chat_id": query.message.chat.id})
            if not is_Rachit:
                Rachit.insert_one({"chat_id": query.message.chat.id})
...
async def Rachitai(client: Client, message: Message):
   ...
   if not message.reply_to_message:
       Rachitdb = MongoClient(MONGO_URL)
       Rachit = Rachitdb["RachitDb"]["Rachit"] 
       is_Rachit = Rachit.find_one({"chat_id": message.chat.id})
       if not is_Rachit:
...
async def Rachitstickerai(client: Client, message: Message):
...
async def Rachitprivate(client: Client, message: Message):
...
async def Rachitprivatesticker(client: Client, message: Message):
